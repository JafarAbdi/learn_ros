...
class TangentBug:
    ...
    def __init__(self):
        ...
        self.angles = np.linspace(self.angle_min, self.angle_max,
                                  667)  # the "667" from robot specification

        # Action Client
        """
        @todo: Uncomment
        self.ClientGTP = actionlib.SimpleActionClient('go_to_point',
                                                      GoToPointAction)
    ...
    def lsCB(self, msg):
        ...
        if self.angles is not None:
            ...
            idx = np.where(ranges_np != np.inf)[
                0]  # the index zero is becasue np.where returns a tuple (.,)
            ranges_ = self.transform_range_stor(ranges_np[idx],
                                                self.angles[idx])
    ...
    def transform_range_stor(self, x, angles):
        ...
        """
        vals_wrt_robot = map(
            lambda angle, z: list(np.dot(self.Rz(angle), [z, 0])), angles, x)
    ...
    def transform_range_rtow(self, data):
        ...
        vals_wrt_world = map(lambda pt: list(np.dot(T, [pt[0], pt[1], 1])),
                             data)
    ...
    def get_heading_pt(self, step_size=0.005, T_inf=True):
        ...
        for i in range(num_steps):
            if check_colision(pt, goal):
                return goal
            if check_colision(pt, pts):
                if T_inf:
    ...
    def get_endpoints(self, eps=0.35):
        ...
        Z[Z == np.
          inf] = 100  # Could be any number which is bigger than self.range_max
        ...
        Os_ = self.transform_range_stor(
            np.array(Os)[infIdx],
            np.array(Angles)[infIdx])
    ...
    def get_normal(pts):
        """
        @deprecated:
        @param pts: points to calculate normal to their
                    least sqaure error minimizer line
        @type numpy.ndarray
    ...
    def get_pts(vals, idx, num_pts=1):
        ...
        @type   numpy.ndarray
        @param  idx index of the points to return the symmetric
                points(num_pts) around it
        @type   int
        @param  num_pts number of points to return the value around idx
        @type   int
        @return [x, y] coordinates of the points within idx - num_pts
                and idx + num_pts
        @rtype  numpy.ndarray
        """
        lower_bound = 0 if (idx - num_pts) < 0 else (idx - num_pts)
        upper_bound = vals.shape[-1] if (
            idx + num_pts + 1) >= vals.shape[0] else (idx + num_pts + 1)
    ...
    def Rz(angle):
        ...
        @rtype  numpy.ndarray
        """
        return tf.transformations.euler_matrix(0, 0, angle)[:2, :2]
    ...
    def Twr(angle, q):
        ...
        """
        T = np.dot(
            tf.transformations.translation_matrix([q[0], q[1], 0.]),
            tf.transformations.euler_matrix(0, 0, angle))[:3, :3]
        T[:2, -1] = np.dot(
            tf.transformations.translation_matrix([q[0], q[1], 0.]),
            tf.transformations.euler_matrix(0, 0, angle))[:2, -1]
        return T
    ...
    def impl(self):
        ...
        while True and not rospy.is_shutdown():
            ...
            Oi = Oi - self.robot_r * (Oi - q) / norm(
                Oi - q, axis=1, keepdims=True)
            ...
            if robot_state == "FB":
                ...
                if d_follow > d_reach:
                    ...
                    follow_dir = np.dot(
                        self.Rz(np.pi / 2),
                        (q - closest_pt) / norm(closest_pt - q))
    ...
    def plot_state(self, pts, e_pts, hd):
        ...
        follow_dir = np.dot(
            self.Rz(np.pi / 2),
            (self.q - closest_pt) / norm(closest_pt - self.q))
...
def check_colision(x, pts, eps=0.035):
    """
    Check if any of the pts are in inside a ball with eps
